  ...
  <div id="certificate">
    ...
    <template v-if="isPromBez">
      <div style="font-family:'Times New Roman',serif; font-size:12pt; color:#000; max-width:800px; margin:0 auto; border:2px solid #333;">
        ...
        <!-- Main 2-column body -->
        <div class="cert-grid" style="border-top:2px solid #333;">

          <!-- LEFT PANEL (Panel 1 — main cert info) -->
          <div class="cert-grid-left cert-grid-left--prombez" style="padding:16px; border-right:2px solid #333;">
    ...
    <template v-else-if="isPtm">
      <div style="font-family:'Times New Roman',serif; font-size:11pt; color:#000; max-width:780px; margin:0 auto;">
        ...
        <div class="cert-grid" style="border:2px solid #333; border-top:none;">

          <!-- LEFT: main cert info -->
          <div class="cert-grid-left" style="padding:20px 24px; border-right:2px solid #333;">
...
const _examDateStr = computed(() => props.protocol?.exam_date || props.cert?.issued_date || null)
...
const validUntilYear = computed(() => {
  const d = props.cert?.valid_until
  if (!d) return '20__'
  return new Date(d + 'T00:00:00').getFullYear()
})

const isExpired = computed(() => {
  if (!props.cert?.valid_until) return false
  const today = new Date()
  today.setHours(0, 0, 0, 0)
  return new Date(props.cert.valid_until + 'T00:00:00') < today
})
...
@media (max-width: 600px) {
  ...
  /* On mobile remove the right border from left panel (becomes top border between stacked panels) */
  .cert-grid-left {
    border-right: none !important;
    border-bottom: 2px solid #333;
  }

  /* ПромБез: remove right border set via inline style too */
  .cert-grid-left--prombez {
    border-right: none !important;
    border-bottom: 2px solid #333;
  }
}
